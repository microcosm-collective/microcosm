...
func (m *UpdateOptionType) Insert() (int, error) {
	...
	if err != nil {
		return http.StatusInternalServerError, errors.New(
			fmt.Sprintf("Insert of update preference failed: %v", err.Error()),
		)
	}
...
INSERT INTO update_options (
   ...
   ,send_sms
	...
	if err != nil {
		tx.Rollback()
		return http.StatusInternalServerError, errors.New(
			fmt.Sprintf("Insert of update option failed: %v", err.Error()),
		)
	}
 ...
 WHERE profile_id = $1
   AND update_type_id = $2`,
	...
	if err != nil {
		tx.Rollback()
		return http.StatusInternalServerError, errors.New(
			fmt.Sprintf("Update of update option failed: %v", err.Error()),
		)
	}
...
// GetAlertPreference returns the notification settings (email, sms) for a given
// user and update type.
   ...
   AND uo.update_type_id = $2`,
	...
	if err == sql.ErrNoRows {
		return UpdateOptionType{}, http.StatusNotFound,
			errors.New(
				fmt.Sprintf(
					"Update options for profile ID %d not found",
					profileId,
				),
			)
	} else if err != nil {
...
// GetAlertPreferences retrieves a user's alert preferences for all available
